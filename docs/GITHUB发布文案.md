...
## 八、v1.1 发布（2026-09-11，当前版）

> 完整版见 [`RELEASE_v1.1.md`](./RELEASE_v1.1.md)；这里给的是可直接粘贴的 Release 标题与正文（中文精简版）。

**位置**：Releases → Draft a new release
**Tag**：`v1.1`（**两位版本号**，不是 `v1.1.0`） ｜ **Release title** 填第一条 ｜ 正文填下面那一整块

```text
BBVoxi 1.1 —— 代码质量加固 + 设置界面改版
```

```markdown
**Windows 按住说话的语音输入法，把语音变成光标处的字。**

本版聚焦三件事：修掉一批真实缺陷、双击 exe 可直接唤起后台实例、设置界面重新排版。
...
- 不碰剪贴板（`SendInput` + `KEYEVENTF_UNICODE` 逐字符注入）
...
单元测试 52 → 56 条全部通过（含按键 keyup、服务商参数落地、唤醒事件等新增回归测试）。

## 校验下载完整性

```powershell
Get-FileHash .\BBVoxi-1.1.exe -Algorithm SHA256
```

> SHA-256：`9E5288BE1A49D56F4B12D6C9E2A3E580034303A1C55038CCBA1510C4255890EB`

完整发布说明见 `docs/RELEASE_v1.1.md`。
