...
> **最新版本**：v1.2（2026-09-18，Stable）
> 详细发布说明见 [docs/RELEASE_v1.2.md](./docs/RELEASE_v1.2.md)，所有历史版本见 [docs/版本历史.md](./docs/版本历史.md)。

按住一个全局快捷键说话，松开后识别结果**自动打到光标所在的位置**——就像输入法正常打字一样，没有多余窗口。

- **单文件**：`BBVoxi-1.2.exe`，无需安装，双击即用
...
从 Releases 下载 `BBVoxi-1.2.exe` → 双击运行 → 托盘出现图标 → 首次运行会弹出设置窗；已在运行时再次双击即弹出设置窗。

### 从源码构建

```bash
cargo build --release     # 产物：target/release/bbvoxi.exe（约 8 MB）
cargo test                # 单元测试（82 条）
```
...
- 录音开始时记住前台窗口，**中途切走就彻底放弃自动输入**（一个字都不打）：
  否则补打会落到别的程序里，退格更是会删掉别人窗口里的内容。
...
```bash
cargo build --release
copy target\release\bbvoxi.exe dist\BBVoxi-1.2.exe
```

然后在 GitHub 新建 Release：Tag `v1.2`，标题 `BBVoxi 1.2`，上传 `dist/BBVoxi-1.2.exe`。
（对外发布统一用两位版本号 `1.2`；`Cargo.toml` 内为满足 Cargo 的 SemVer 字段限制写 `1.2.0`。）
可直接复制粘贴的 Release 标题与正文见 [docs/GITHUB发布文案-v1.2.md](./docs/GITHUB发布文案-v1.2.md)。
...
| 文字打到别处 / 没出现 | 录音开始时本程序会收起自己的设置窗让焦点回到你的程序；注入前也会再次确认前台窗口不是自己。**中途切走过窗口的话，本次结果会被安全放弃**（一个字都不打，界面会提示）—— 这是防止打错地方的设计 |
| 结果没打出来，提示"切换了窗口" | 录音开始后切过窗口：此时往新窗口写字（尤其是纠错退格）会破坏你正在编辑的内容，所以本次不自动输入。结果仍会保存在设置窗的「最近识别」里 |
...
- **录音中途切走窗口 → 本次不自动输入**：这是"不打错地方"的安全设计（切走后再往新窗口写字，尤其是纠错退格，会破坏你正在编辑的内容）。结果仍会留在设置窗的「最近识别」里，日志也有完整记录。两种「边说话边打字」设置下的行为一致
...
| 文档 | 说明 |
|---|---|
| [docs/RELEASE_v1.2.md](./docs/RELEASE_v1.2.md) | **当前版本**：包信息（SHA-256/大小/构建时间） · 功能清单 · 技术要点 · 14 模块 · 已知限制 · 升级指南 · 安全声明 · 发布检查表 |
| [docs/全量审查与修复报告-v1.2.md](./docs/全量审查与修复报告-v1.2.md) | 本轮的逐项根因分析、验证方式（TDD + 变异检查）与过程中出现的问题 |
| [docs/GITHUB发布文案-v1.2.md](./docs/GITHUB发布文案-v1.2.md) | GitHub Release 标题与正文（复制粘贴用） |
